...
class SocialServiceManager:
    ...
    def display_host_info(self):
        ...
        print("🎭 功能说明:")
        print("   • 主设备提供音乐生成服务")
        print("   • 主设备可输出自己的情绪数据（默认启用）")
        print("   • 客户端连接后一起融合情绪生成音乐")
        print()
    ...
    def start_local_brain_processor(self):
        ...
        # 默认启用本地EEG设备，除非用户明确拒绝
        use_local_eeg = input("主设备是否使用本地EEG设备输出情绪？ (Y/n，默认Y): ").strip().lower()
        
        # 默认为启用，只有明确输入n/no才禁用
        if use_local_eeg in ['n', 'no', '否']:
            print("⏭️  跳过本地EEG设备，仅作为音频服务器运行")
            print("💡 提示：主设备将不会输出自己的情绪数据")
            return True
        else:
            print("🧠 启动本地脑波数据处理服务...")
            try:
                ...
                print(f"👤 主设备用户ID: {self.local_user_id}")
                print("🎭 主设备的情绪将与其他用户融合生成音乐")
                
                return True
            except Exception as e:
                print(f"❌ 启动本地脑波处理服务失败: {e}")
                print("⚠️  主设备将仅作为音频服务器运行")
                return False
    ...
    def run(self):
        ...
        if not self.start_local_brain_processor():
            print("⚠️  本地脑波处理服务启动失败，主设备将仅作为音频服务器")
        ...
        print("🔗 等待客户端用户连接...")
        if self.local_user_id:
            print("🎧 请戴上你的Emotiv EEG设备（主设备用户）")
            print("🎭 主设备将输出情绪数据参与融合")
        else:
            print("💡 主设备仅作为音频服务器运行")
